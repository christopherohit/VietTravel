...
namespace VietTravel
{
    ...
    public partial class TrangChu : Form
    {
               ...
               for (int i =0; i < PrettyGribView.Rows.Count; i++)
                {
                    ...
                    if (PrettyGribView.Rows[i].Selected == true)
                    {
                        ...
                        var res = MessageBox.Show($"Do you want to remove Employee {row.Cells[0].Value.ToString()} from the system", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                        switch (res)
                        {
                            case DialogResult.Yes:
                                ...
                                sqlCommand.Parameters.AddWithValue("@emails", row.Cells[4].Value.ToString());
        ...
        private void SavePretty_Click(object sender, EventArgs e)
        {
            ...
            command.Parameters.AddWithValue("@anhhuongdan", rawdata);
            command.ExecuteNonQuery();
            Pretty.Image = Image.FromStream(new MemoryStream((byte[])rawdata));
...
/*            open.Filter = "Image Files(*.JPG ; *.jpeg ; *.gif ; *.bmp ; *.png) | *.jpg ; *.jpeg ; *.gif ; *.bmp ; *png";*/
            ...
            {
                Pretty.Image = new Bitmap(open.FileName);
            }
        }
        ...
        private void SaveKT_Click(object sender, EventArgs e)
        {
            if (IsDirty == false)
            {
                /*(select * FROM OPENROWSET(BULK @anhthe, SINGLE_BLOB) AS picture)*/
                FileStream fs;
                byte[] rawdata;
                fs = new FileStream(openFileDialog1.FileName, FileMode.Open, FileAccess.Read);
                rawdata = new byte[Convert.ToInt32(fs.Length)];
                ...
                command.Parameters.AddWithValue("@anhthe", rawdata);
